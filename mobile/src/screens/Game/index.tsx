import { useEffect, useState } from 'react';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRoute, useNavigation } from '@react-navigation/native'; // para buscar as informações da que vem na rota
import { TouchableOpacity, View, Image, FlatList, Text } from 'react-native';
import { GameParams } from '../../@types/navigation';
import { Entypo } from '@expo/vector-icons';

import { Background } from '../../components/Background';
import { Heading } from '../../components/Heading';
import { DouCard, DouCardProps } from '../../components/DouCard';
import { DuoMath } from '../../components/DuoMath';

import logoImg from '../../assets/logo-nlw-esports.png'
import { styles } from './styles';
import { THEME } from '../../theme';

export function Game() {
  const [duos, setDuos] = useState<DouCardProps[]>([]);
  const [discordDouSelected, setDiscordDouSelected] = useState<string>('');

  const navigation = useNavigation();
  const route = useRoute();
  const game = route.params as GameParams;

  function handleGoBack() {
    navigation.goBack(); // voltar a página anterior
  };

  async function getDiscordUser(adsId: string) {
    fetch(`http://192.168.0.104:3333/ads/${adsId}/discord`)
      .then(res => res.json())
      .then(data => {
        setDiscordDouSelected(data.discord)
      })
  };

  useEffect(() => {
    fetch(`http://192.168.0.104:3333/games/${game.id}/ads`)
      .then(res => res.json())
      .then(data => setDuos(data))
  }, []);

  return (
    <Background>
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity onPress={handleGoBack}>
            <Entypo
              name="chevron-thin-left" // nome do ícone
              color={THEME.COLORS.CAPTION_300}
              size={20}
            />
          </TouchableOpacity>

          <Image
            source={logoImg}
            style={styles.logo}
          />

          <View style={styles.right} />
        </View>

        <Image
          source={{ uri: game.bannerUrl }}
          style={styles.cover}
          resizeMode="cover"
        />

        <Heading
          title={game.titles}
          subtitle="Conecte-se e comece a jogar!"
        />

        <FlatList
          data={duos}
          keyExtractor={item => item.id}
          renderItem={({ item }) => (
            <DouCard
              data={item}
              onConnect={() => getDiscordUser(item.id)}
            />
          )}
          horizontal
          style={styles.containerList}
          contentContainerStyle={[duos.length > 0 ? styles.contentList : styles.emptyListContent]}
          showsHorizontalScrollIndicator={false}
          ListEmptyComponent={() => ( // caso a lista esteja vazia
            <Text style={styles.emptyListText}>
              Não há anúncios publicados ainda.
            </Text>
          )}
        />

        <DuoMath
          visible={discordDouSelected.length > 0}
          discord={discordDouSelected}
          onClose={() => setDiscordDouSelected('')}
        />
      </SafeAreaView>
    </Background>
  );
}